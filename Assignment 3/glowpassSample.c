...
void Read_BMP_Data(char *filename,int *h,int *w,BMP *bmp)
{
	...
	char hex[3][3]; //hex[0] means R hex[1] means G hex[2] means B
	output[0]=fopen("string0.sh","w");
	output[1]=fopen("string1.sh","w");
	output[2]=fopen("string2.sh","w");
	if(output[0]==NULL || output[1]==NULL || output[1]==NULL)
	{
...
void write_BMP_Data(char *filename,int *h,int *w,BMP *bmp){
	...
	outputfinal[0]=fopen("string00.txt","r");
	outputfinal[1]=fopen("string11.txt","r");
	outputfinal[2]=fopen("string22.txt","r");
